    ...
    <div style="background-color: #f6f8fb">
      <ul class="">
        <div v-for="(item, key) in list" :key="key">
          <div class="list" v-if="item.type === 'boardList'">
            <li class="card" :key="key" @click="goIssuePage(item.issue_idx)">
              <dl>
                <div
                  style="
                    display: flex;
...
<script>
  ...
  // import person from '@/static/images/common/person.svg'
  // import chattingBox from '@/static/images/common/chattingBox.svg'
  // import emptyProfile from '@/static/images/common/image.svg'
  // import profile from '@/static/images/common/profile.svg'
    ...
    methods: {
                  ...
                  list = list.map(value => {
                    ...
                    if (v === 'viewCount') {
                      list = list.sort((a, b) => {
                        // console.log(a.dist)
                        // console.log(b.dist)
                        if (new Date(a.view_count) > new Date(b.view_count))
                          return -1
                        if (new Date(a.view_count) == new Date(b.view_count))
                          return 0
                        if (new Date(a.view_count) < new Date(b.view_count))
      ...
      async getIssueList() {
        try {
          ...
          await api
            .get(`board/open/board/view/${this.issuePageNo}`, config)
            .then(result => {
              console.log(result)
              if (result.data.success) {
                if (
                  ...
                  list = list.map(value => {
                    ...
                    value.type = 'boardList'
                    return value
                  })
                  this.issuePageNo = this.issuePageNo + 1
                  this.issueList = list
      ...
      mergeSort() {
        ...
        this.list = this.list.sort((a, b) => {
          let temp_a = parseInt(a.view_count)
          let temp_b = parseInt(b.view_count)
          if (temp_a < temp_b) return 1
          if (temp_a == temp_b) return 0
          if (temp_a > temp_b) return -1
        })
